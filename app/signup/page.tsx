...
function Eyeball({
  ...
  return (
    ...
    >
      <motion.div
        animate={isPasswordFocused ? { rotateX: 0, rotateY: 0 } : { rotateX: tilt.rotateX, rotateY: tilt.rotateY }}
      ...
      >
        ...
        >
          ...
          <motion.div
            animate={{ x: isPasswordFocused ? 0 : pupil.x * 0.65, y: isPasswordFocused ? 0 : pupil.y * 0.65 }}
          ...
          <motion.div
            ...
            style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: "50%", background: "linear-gradient(0deg, #f8f4ee 0%, #e8ddd0 100%)", borderRadius: "0 0 50% 50% / 0 0 100% 100%", transformOrigin: "bottom center", zIndex: 10 }}
          />
...
function MobileHero({ isPasswordFocused }: { isPasswordFocused: boolean }) {
  return (
    <div
      ...
      style={{ background: "linear-gradient(145deg, #FF9A3C 0%, #FF6B00 65%, #E05500 100%)", minHeight: 220 }}
    >
      {[
        ...
        <motion.div
          ...
          animate={{
            borderRadius: ["60% 40% 70% 30% / 50% 60% 40% 70%", "40% 60% 30% 70% / 70% 40% 60% 30%", "60% 40% 70% 30% / 50% 60% 40% 70%"],
            scale: [1, 1.08, 1],
          }}
...
const getStrength = (pwd: string) => {
  ...
  if (pwd.match(/[A-Z]/) && pwd.match(/[0-9]/) && pwd.length >= 10) return { score: 4, label: "Strong", color: "#22c55e" };
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col lg:flex-row">
      ...
      <motion.div
        ...
        className="flex-1 lg:w-[46%] flex flex-col justify-center px-6 sm:px-10 md:px-14 lg:px-16 py-8 lg:py-12 overflow-y-auto"
      >
        {/* Brand desktop */}
        <motion.div
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}>
          <h1 className="text-3xl lg:text-4xl font-bold text-gray-900 mb-1.5 tracking-tight">Create account</h1>
          <p className="text-gray-400 text-sm mb-7">Start for free. No credit card required.</p>
        </motion.div>
        ...
        <AnimatePresence>
          ...
          {success && (
            <motion.div
              ...
              animate={{ opacity: 1, height: "auto", marginBottom: 14 }}
              exit={{ opacity: 0, height: 0, marginBottom: 0 }}
              className="bg-green-50 border border-green-100 text-green-600 text-sm rounded-2xl px-4 py-3"
            >
              {success}
            </motion.div>
          )}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
            ...
            <input id="name" type="text" value={name} onChange={(e) => setName(e.target.value)} required placeholder="John Doe" className={inputClass} />
          </motion.div>

          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }}>
            <label className="block text-sm font-medium text-gray-600 mb-2">Email</label>
            <input id="signup-email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} required placeholder="you@example.com" className={inputClass} />
          </motion.div>
                  ...
                  <div className="flex gap-1.5 mb-1.5">
                    {[1, 2, 3, 4].map((i) => (
                      <motion.div
                        ...
                        style={{ background: i <= strength.score ? strength.color : "#e5e7eb", transformOrigin: "left" }}
                      />
                    ))}
                  </div>
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.45 }} className="pt-1">
            <motion.button
              id="signup-btn"
              type="submit"
              disabled={loading}
        ...
        <p className="text-center text-sm text-gray-400">
          ...
          <Link href="/login" className="text-orange-500 font-semibold hover:text-orange-600 transition-colors">Sign in</Link>
        </p>
      </motion.div>
      ...
      >
        ...
        />
        {[{ size: 300, top: "-10%", left: "-8%", delay: 0 }, { size: 200, bottom: "-8%", right: "-5%", delay: 1.5 }, { size: 120, top: "60%", left: "10%", delay: 0.8 }].map((b, i) => (
          <motion.div
            key={i}
            className="absolute bg-white/10"
            style={{ width: b.size, height: b.size, top: (b as any).top, left: (b as any).left, bottom: (b as any).bottom, right: (b as any).right }}
            animate={{ borderRadius: ["60% 40% 70% 30% / 50% 60% 40% 70%", "40% 60% 30% 70% / 70% 40% 60% 30%", "60% 40% 70% 30% / 50% 60% 40% 70%"], scale: [1, 1.1, 1] }}
